...
description: Internationalization (i18n) rules for the OpenJustice React frontend — how translation files are organized (per-feature, mirroring the folder structure), translation key naming conventions, the centralized i18n service wrapper (`@/lib/i18n`), where the `t()` function is called (UI primarily, sometimes container for derived strings), interpolation and pluralization patterns, locale-aware number and date formatting via `Intl`, RTL considerations, and the rules that prevent the most common i18n bugs (string concatenation, full-sentence keys, hard-coded English fallbacks). Use this skill whenever adding a new translatable string, naming a translation key, formatting a number or date for display, structuring a new feature's translation file, or whenever the user asks "how should this be translated", "where do translation files go", "how do I handle the plural", "should I concatenate these strings", or anything about i18n. Trigger this skill any time user-facing text or locale-aware formatting is being added — i18n bugs are almost never caught in development (you only see them in the locale you develop in) and concatenated strings are the most common cause of unfixable translations.
---
